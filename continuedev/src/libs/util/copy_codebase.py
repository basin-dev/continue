...
def calculate_diff(filepath: str, original: str, updated: str) -> List[FileEdit]:
	...
	for tag, i1, i2, j1, j2 in s.get_opcodes():
		...
		if tag == "equal":
			...
			edits.append(FileEdit.from_deletion(filepath, Range.from_indices(original, i1, i2)))
			offset -= i2 - i1
		elif tag == "insert":
			edits.append(FileEdit.from_insertion(filepath, Position.from_index(original, i1), replacement))
			offset += j2 - j1
		elif tag == "replace":
			edits.append(FileEdit(filepath, Range.from_indices(original, i1, i2), replacement))
			offset += (j2 - j1) - (i2 + i1)
		else:
...
class CopyCodebaseEventHandler(PatternMatchingEventHandler):
		...
		def _event_to_edit(self, event) -> FileSystemEdit | None:
...
def maintain_copy_workspace(agent: Agent, filesystem: FileSystem, orig_root: str, copy_root: str):
	observer = Observer()
	event_handler = CopyCodebaseEventHandler([".git"], [], agent, orig_root, copy_root, filesystem)
