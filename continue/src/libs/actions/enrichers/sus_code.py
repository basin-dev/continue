...
from ..main import Agent, Artifact
from ....models.main import Traceback, RangeInFile
import pathspec
...
def fl_call_graph(tb: Traceback, filesystem: FileSystem=VirtualFileSystem({})) -> List[RangeInFile]:
    ...
    call_graph = create_call_graph(tb.frames[-1].function, range_in_file, filesystem=filesystem)
...
def fl2(tb: Traceback, query: str, filesystem: FileSystem=VirtualFileSystem({}), n: int = 4) -> List[RangeInFile]:
    ...
    filtered_ranges = [frame_to_code_range(frame, filesystem=filesystem) for frame in filtered_frames]
