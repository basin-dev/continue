...
class Agent(BaseModel):
    ...
    def run_from_step(self, step: Step, require_permission: bool=False):
        ...
        # Make it a generator!
        while not isinstance(next_step, DoneStep):
            output = runner.run(next_step) # Should the runner be the thing keeping track of history from outputs?
...
class DemoAgent(Agent):
    def __init__(self):
        super().__init__(llm=OpenAI(api_key), filesystem=RealFileSystem(), policy=DemoPolicy())
